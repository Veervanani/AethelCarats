...
export const ensureStorefrontCmsSeeded = async () => {
  try {
    // 1. SEED MEGA MENU CARDS IF EMPTY
    const cardCount = await prisma.megaMenuCard.count();
    if (cardCount === 0) {
      console.log('Seeding initial storefront Mega Menu promotional cards...');
      await prisma.megaMenuCard.createMany({
        data: [
          {
            categorySlug: 'rings',
            ...
            imageUrl: '/assets/floksy_rings_cat.png',
          ...
          },
          {
            ...
            imageUrl: '/assets/floksy_rings_cat_2.png',
          ...
          },
          {
            ...
            imageUrl: '/assets/floksy_earrings_cat.png',
          ...
          },
          {
            ...
            imageUrl: '/assets/floksy_pendants_cat.png',
          ...
          },
          {
            ...
            imageUrl: '/assets/floksy_diamonds_cat.png',
            targetUrl: '/diamonds',
            sortOrder: 1,
            isEnabled: true,
          },
    ...
    const fgCount = await prisma.filterGroup.count();
    if (fgCount === 0) {
      console.log('Seeding initial Jewellery Filter Groups & Options...');

      // GENDER
      const genderGroup = await prisma.filterGroup.create({
        data: {
          key: 'gender',
          label: 'Gender',
      ...
      for (const styleCfg of categoryStyleConfigs) {
        const sg = await prisma.filterGroup.create({
          data: {
            key: styleCfg.key,
      ...
      // METAL
      const metalGroup = await prisma.filterGroup.create({
        data: {
          key: 'metal',
          label: 'Metal & Purity',
          targetType: 'JEWELLERY',
      ...
      // STONE SHAPE
      const shapeGroup = await prisma.filterGroup.create({
        data: {
          key: 'shape',
          label: 'Stone Shape',
          targetType: 'JEWELLERY',
      ...
      // DIAMOND ORIGIN
      const diamondGroup = await prisma.filterGroup.create({
        data: {
          key: 'diamondType',
    ...
    const shapeGroups = await prisma.filterGroup.findMany({
      where: {
    ...
    for (const sg of shapeGroups) {
      const existingOptions = await prisma.filterOption.findMany({ where: { filterGroupId: sg.id } });
    ...
    await prisma.filterGroup.updateMany({
    ...
    for (const styleCfg of categoryStyleConfigs) {
      let sg = await prisma.filterGroup.findFirst({
        where: {
          key: styleCfg.key,
          categoryScope: styleCfg.scope,
        },
      ...
      if (!sg) {
        ...
        sg = await prisma.filterGroup.create({
          data: {
            key: styleCfg.key,
            label: styleCfg.label,
            categoryScope: styleCfg.scope,
    ...
    const filterConfigCount = await prisma.diamondFilterConfig.count();
    if (filterConfigCount === 0) {
      console.log('Seeding initial Diamond Vault Filter configurations and options...');

      // TYPE
      const typeConfig = await prisma.diamondFilterConfig.create({
        data: {
          key: 'type',
      ...
      await prisma.diamondFilterOption.createMany({
        data: [
      ...
      // CLASSIFICATION
      const classConfig = await prisma.diamondFilterConfig.create({
        data: {
          key: 'classification',
      ...
      // SHAPES
      const shapeConfig = await prisma.diamondFilterConfig.create({
        data: {
          key: 'shape',
          title: 'Stone Shape',
          filterType: 'SHAPE',
      ...
      // CARAT WEIGHT BOUNDS
      await prisma.diamondFilterConfig.create({
        data: {
          key: 'carat',
          title: 'Carat Weight',
          filterType: 'RANGE',
      ...
      // COLOR GRADE
      const colorConfig = await prisma.diamondFilterConfig.create({
        data: {
          key: 'color',
          title: 'Color Grade',
          filterType: 'COLOR',
      ...
      // CLARITY
      const clarityConfig = await prisma.diamondFilterConfig.create({
        data: {
          key: 'clarity',
          title: 'Clarity Grade',
          filterType: 'CLARITY',
      ...
      // CERTIFICATION
      const certConfig = await prisma.diamondFilterConfig.create({
        data: {
          key: 'certification',
    ...
    const reviewCount = await (prisma as any).homepageReview.count();
    if (reviewCount === 0) {
      console.log('Seeding initial Homepage Customer Reviews...');
      await (prisma as any).homepageReview.createMany({
        data: [
          {
            customerName: 'Ryan K.',
            rating: 5,
