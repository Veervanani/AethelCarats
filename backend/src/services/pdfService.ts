...
export function drawHeader(doc: typeof PDFDocument.prototype, options: LuxuryPdfOptions) {
  // Brand Header
  doc
...
export function drawFooter(doc: typeof PDFDocument.prototype, currentPage: number, totalPages: number) {
...
export async function generateOrderStatementPdf(order: any, res: Response) {
  ...
  if (successfulPayments.length === 0) {
    ...
    successfulPayments.forEach((pay: any) => {
      if (isRowEven) {
  ...
  const totalRefunded = (order.refunds || []).reduce((acc: number, r: any) => acc + r.amount, 0);
  ...
  if (order.discount > 0) drawSummaryLine('Discount:', `-${formatCurrency(order.discount, order.currency)}`);
  if (order.tax > 0) drawSummaryLine('Tax:', formatCurrency(order.tax, order.currency));
  if (order.shippingFee > 0) drawSummaryLine('Shipping Fee:', formatCurrency(order.shippingFee, order.currency));
  ...
  if (totalRefunded > 0) drawSummaryLine('Total Refunded:', `-${formatCurrency(totalRefunded, order.currency)}`);
...
export async function generatePaymentReceiptPdf(payment: any, order: any, res: Response) {
  const doc = new PDFDocument({ size: 'A4', margin: 40 });
  ...
  rows.forEach((r, idx) => {
    if (idx % 2 === 1) doc.rect(40, y - 2, 515, 20).fill(LIGHT_BG);
...
export async function generateCustomerStatementPdf(customer: any, orders: any[], fromDate?: string, toDate?: string, res?: Response) {
  const doc = new PDFDocument({ size: 'A4', margin: 40 });
  ...
  orders.forEach((o) => {
    ...
    const paid = (o.payments || []).filter((p: any) => p.status === 'SUCCESS').reduce((a: number, p: any) => a + p.amount, 0);
    totalPaid += paid;
  });
  ...
  orders.forEach((ord) => {
    ...
    if (isRowEven) doc.rect(40, y - 2, 515, 20).fill(LIGHT_BG);
    doc.fillColor(DARK).font('Helvetica').fontSize(8.5);
    ...
    (ord.payments || []).filter((p: any) => p.status === 'SUCCESS').forEach((pay: any) => {
...
export async function generateFinancialStatementPdf(title: string, metrics: any, rows: any[], periodStr: string, res: Response) {
  const doc = new PDFDocument({ size: 'A4', margin: 40 });
...
export async function generateOrderInvoicePdf(order: any, res: Response) {
  const doc = new PDFDocument({ size: 'A4', margin: 40 });
  ...
  (order.items || []).forEach((item: any) => {
    if (isRowEven) doc.rect(40, y - 2, 515, 22).fill(LIGHT_BG);
