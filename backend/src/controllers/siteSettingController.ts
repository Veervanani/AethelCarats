...
export const getSiteSettings = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (dbMap.footer_settings && typeof dbMap.footer_settings === 'object') {
      if (dbMap.footer_settings.brandName) result.storeName = dbMap.footer_settings.brandName;
      if (dbMap.footer_settings.email) result.contactEmail = dbMap.footer_settings.email;
      if (dbMap.footer_settings.phone) result.contactPhone = dbMap.footer_settings.phone;
      if (dbMap.footer_settings.address) result.storeAddress = dbMap.footer_settings.address;
      if (dbMap.footer_settings.instagram) result.instagramUrl = dbMap.footer_settings.instagram;
      if (dbMap.footer_settings.facebook) result.facebookUrl = dbMap.footer_settings.facebook;
      if (dbMap.footer_settings.pinterest) result.pinterestUrl = dbMap.footer_settings.pinterest;
      result.footer_settings = dbMap.footer_settings;
    }
    if (dbMap.footer_config && typeof dbMap.footer_config === 'object') {
      if (dbMap.footer_config.contactEmail) result.contactEmail = dbMap.footer_config.contactEmail;
      if (dbMap.footer_config.contactPhone) result.contactPhone = dbMap.footer_config.contactPhone;
      if (dbMap.footer_config.address) result.storeAddress = dbMap.footer_config.address;
...
export const updateSiteSetting = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (key === 'site_settings' && typeof value === 'object' && value !== null) {
      ...
      await Promise.all([
        prisma.siteSetting.upsert({ where: { key: 'whatsappNumber' }, update: { value: waNumber }, create: { key: 'whatsappNumber', value: waNumber } }),
        prisma.siteSetting.upsert({ where: { key: 'whatsappDisplayNumber' }, update: { value: waDisplay }, create: { key: 'whatsappDisplayNumber', value: waDisplay } }),
        prisma.siteSetting.upsert({ where: { key: 'whatsappDefaultMessage' }, update: { value: waMessage }, create: { key: 'whatsappDefaultMessage', value: waMessage } }),
        prisma.siteSetting.upsert({ where: { key: 'whatsapp_config' }, update: { value: JSON.stringify(waConfig) }, create: { key: 'whatsapp_config', value: JSON.stringify(waConfig) } }),

        prisma.siteSetting.upsert({ where: { key: 'consultPhone' }, update: { value: consultPhone }, create: { key: 'consultPhone', value: consultPhone } }),
        prisma.siteSetting.upsert({ where: { key: 'consultTitle' }, update: { value: consultTitle }, create: { key: 'consultTitle', value: consultTitle } }),
        prisma.siteSetting.upsert({ where: { key: 'consultDescription' }, update: { value: consultDesc }, create: { key: 'consultDescription', value: consultDesc } }),
        prisma.siteSetting.upsert({ where: { key: 'consultPhoneLabel' }, update: { value: consultPhoneLbl }, create: { key: 'consultPhoneLabel', value: consultPhoneLbl } }),
        prisma.siteSetting.upsert({ where: { key: 'consultEmail' }, update: { value: consultEmail }, create: { key: 'consultEmail', value: consultEmail } }),
        prisma.siteSetting.upsert({ where: { key: 'consultEmailLabel' }, update: { value: consultEmailLbl }, create: { key: 'consultEmailLabel', value: consultEmailLbl } }),
        prisma.siteSetting.upsert({ where: { key: 'consultCloseLabel' }, update: { value: consultCloseLbl }, create: { key: 'consultCloseLabel', value: consultCloseLbl } }),
        prisma.siteSetting.upsert({ where: { key: 'enableConsultAtelierExpert' }, update: { value: enableConsult }, create: { key: 'enableConsultAtelierExpert', value: enableConsult } }),
        prisma.siteSetting.upsert({ where: { key: 'consult_expert_config' }, update: { value: JSON.stringify(consultConfig) }, create: { key: 'consult_expert_config', value: JSON.stringify(consultConfig) } }),
      ]);

      if (cleanPayload.contactPhone) {
        await prisma.siteSetting.upsert({ where: { key: 'contactPhone' }, update: { value: String(cleanPayload.contactPhone) }, create: { key: 'contactPhone', value: String(cleanPayload.contactPhone) } });
        await prisma.siteSetting.upsert({ where: { key: 'company_phone' }, update: { value: String(cleanPayload.contactPhone) }, create: { key: 'company_phone', value: String(cleanPayload.contactPhone) } });
      }
      if (cleanPayload.contactEmail) {
        await prisma.siteSetting.upsert({ where: { key: 'contactEmail' }, update: { value: String(cleanPayload.contactEmail) }, create: { key: 'contactEmail', value: String(cleanPayload.contactEmail) } });
      }
      if (cleanPayload.storeName) {
        await prisma.siteSetting.upsert({ where: { key: 'storeName' }, update: { value: String(cleanPayload.storeName) }, create: { key: 'storeName', value: String(cleanPayload.storeName) } });
      }
      if (cleanPayload.storeAddress) {
        await prisma.siteSetting.upsert({ where: { key: 'storeAddress' }, update: { value: String(cleanPayload.storeAddress) }, create: { key: 'storeAddress', value: String(cleanPayload.storeAddress) } });
      }
      if (cleanPayload.publicSiteUrl) {
        await prisma.siteSetting.upsert({ where: { key: 'publicSiteUrl' }, update: { value: String(cleanPayload.publicSiteUrl) }, create: { key: 'publicSiteUrl', value: String(cleanPayload.publicSiteUrl) } });
      }
    ...
    if (key === 'footer_settings' && typeof value === 'object' && value !== null) {
      ...
      if (value.phone) {
        ...
        await prisma.siteSetting.upsert({ where: { key: 'company_phone' }, update: { value: String(value.phone) }, create: { key: 'company_phone', value: String(value.phone) } });
      }
      if (value.address) {
    ...
    if (key === 'consult_expert_config' || key === 'consultPhone' || key === 'consultTitle') {
      if (key === 'consult_expert_config' && typeof value === 'object' && value !== null) {
        ...
        await Promise.all([
          prisma.siteSetting.upsert({ where: { key: 'consultPhone' }, update: { value: phone }, create: { key: 'consultPhone', value: phone } }),
          prisma.siteSetting.upsert({ where: { key: 'consultTitle' }, update: { value: title }, create: { key: 'consultTitle', value: title } }),
        ]);
        if (value.consultDescription) {
          await prisma.siteSetting.upsert({ where: { key: 'consultDescription' }, update: { value: String(value.consultDescription) }, create: { key: 'consultDescription', value: String(value.consultDescription) } });
        }
        if (value.consultPhoneLabel) {
          await prisma.siteSetting.upsert({ where: { key: 'consultPhoneLabel' }, update: { value: String(value.consultPhoneLabel) }, create: { key: 'consultPhoneLabel', value: String(value.consultPhoneLabel) } });
        }
        if (value.enableConsultAtelierExpert !== undefined) {
          await prisma.siteSetting.upsert({ where: { key: 'enableConsultAtelierExpert' }, update: { value: String(value.enableConsultAtelierExpert) }, create: { key: 'enableConsultAtelierExpert', value: String(value.enableConsultAtelierExpert) } });
        }
      } else if (key === 'consultPhone') {
        const phoneStr = String(value);
        try {
          const existing = await prisma.siteSetting.findUnique({ where: { key: 'consult_expert_config' } });
          if (existing && existing.value) {
            const parsed = JSON.parse(existing.value);
            parsed.consultPhone = phoneStr;
            await prisma.siteSetting.update({ where: { key: 'consult_expert_config' }, data: { value: JSON.stringify(parsed) } });
          }
        } catch (e) {}
