...
export const getCommissionsList = async (req: AuthRequest, res: Response) => {
  try {
...
export const updateCommissionPlan = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (Array.isArray(rules)) {
      await prisma.commissionRule.deleteMany({ where: { planId: id } });
    ...
    const refreshed = await prisma.commissionPlan.findUnique({
      where: { id },
      include: { rules: true },
    });
