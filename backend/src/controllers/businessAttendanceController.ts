...
export const getAttendanceList = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const monthStr = (req.query.month as string) || ''; // e.g. "2026-08"
...
export const manualAttendanceEntry = async (req: AuthRequest, res: Response) => {
  try {
    const {
      employeeId,
      date,
...
export const getMonthlyAttendanceReport = async (req: AuthRequest, res: Response) => {
  try {
    const monthStr = (req.query.month as string) || new Date().toISOString().slice(0, 7); // e.g. "2026-08"
    const [year, month] = monthStr.split('-').map(Number);
    const startOfMonth = new Date(Date.UTC(year, month - 1, 1, 0, 0, 0));
    const endOfMonth = new Date(Date.UTC(year, month, 0, 23, 59, 59, 999));
    ...
    const reportData = employees.map((emp) => {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (record) {
          ...
          if (record.lateStatus) lateCount++;
