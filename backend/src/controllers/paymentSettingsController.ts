...
export const getPaymentSettings = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const settingsMap: Record<string, string> = { ...DEFAULT_PAYMENT_SETTINGS };
  ...
  try {
    ...
    const updatePromises = Object.entries(payload).map(async ([key, value]) => {
      ...
      return prisma.siteSetting.upsert({
        ...
        create: { id: randomUUID(), key, value: valStr, updatedAt: new Date() },
      });
    });
    ...
    const freshSettings = await prisma.siteSetting.findMany({
      where: { key: { in: ALL_PAYMENT_KEYS } },
    });
    const result: Record<string, string> = { ...DEFAULT_PAYMENT_SETTINGS };
...
export const getPublicPaymentConfig = async (req: Request, res: Response) => {
  try {
    const publicKeys = [
      'paypal_client_id',
      'paypal_mode',
