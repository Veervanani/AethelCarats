...
const candidateUploadDirs = [
    path_1.default.join(process.cwd(), 'public_html', 'uploads'),
    ...
    path_1.default.join(__dirname, '..', '..', 'public_html', 'uploads'),
...
app.use('/uploads', async (req, res, next) => {
    ...
    const filename = path_1.default.basename(cleanSubpath);
    ...
    try {
        const [mediaRows] = await prisma_1.mysqlPool.query('SELECT data, fileType FROM `Media` WHERE url LIKE ? OR url LIKE ? OR name = ? LIMIT 1', [`%${filename}`, `%${cleanSubpath}`, filename]);
        let row = mediaRows && mediaRows[0];
        if (!row || !row.data) {
            const [piRows] = await prisma_1.mysqlPool.query('SELECT data FROM `ProductImage` WHERE url LIKE ? LIMIT 1', [`%${filename}`]);
        ...
        if (row && row.data && row.data.length > 0) {
            ...
            const targetDiskPath = path_1.default.join(primaryCacheDir, cleanSubpath);
            try {
                const parentDir = path_1.default.dirname(targetDiskPath);
                if (!fs_1.default.existsSync(parentDir))
                    fs_1.default.mkdirSync(parentDir, { recursive: true });
                fs_1.default.writeFileSync(targetDiskPath, row.data);
            }
            catch (cacheErr) {
                console.warn('Cache write notice:', cacheErr?.message);
            }
            const ext = path_1.default.extname(cleanSubpath).toLowerCase();
            const mimeTypes = {
                '.png': 'image/png',
                '.jpg': 'image/jpeg',
                '.jpeg': 'image/jpeg',
