 ...
 */
const SOURCE_BACKUP_PATH = path_1.default.join(__dirname, '../../../Cloud_SQL_Export_2026-08-11 (18_58_11).sql');
const OUTPUT_MYSQL_SQL_PATH = path_1.default.join(__dirname, '../../../hostinger-mysql-migration.sql');
function convertPostgresDumpToMysql() {
    console.log(`🔍 Inspecting PostgreSQL Dump: ${SOURCE_BACKUP_PATH}`);
    if (!fs_1.default.existsSync(SOURCE_BACKUP_PATH)) {
        console.error(`❌ Error: Source backup file not found at ${SOURCE_BACKUP_PATH}`);
        process.exit(1);
    }
    const fileContent = fs_1.default.readFileSync(SOURCE_BACKUP_PATH, 'utf-8');
    ...
    fs_1.default.writeFileSync(OUTPUT_MYSQL_SQL_PATH, mysqlSql, 'utf-8');
