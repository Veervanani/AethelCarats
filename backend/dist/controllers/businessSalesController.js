...
exports.deleteSale = exports.updateSale = exports.createSale = exports.getSaleById = exports.getSalesList = exports.calculateSalesPreview = void 0;
exports.computeAuthoritativeFinancials = computeAuthoritativeFinancials;
const prisma_1 = __importDefault(require("../prisma"));
function computeAuthoritativeFinancials(input) {
...
const calculateSalesPreview = (req, res) => {
    try {
...
exports.calculateSalesPreview = calculateSalesPreview;
const getSalesList = async (req, res) => {
    try {
        ...
        const [total, sales, aggregate] = await Promise.all([
            prisma_1.default.internalSale.count({ where }),
            prisma_1.default.internalSale.findMany({
                where,
                skip,
                take: limit,
            ...
            prisma_1.default.internalSale.aggregate({
                where,
                _sum: {
...
const getSaleById = async (req, res) => {
    try {
        const { id } = req.params;
        const sale = await prisma_1.default.internalSale.findUnique({
            where: { id },
            include: {
                employee: true,
                customer: true,
...
const createSale = async (req, res) => {
    try {
        const { invoiceNo, saleDate = new Date(), customerName, customerCountry, customerId, productType = 'Diamond', productDescription, stoneType, shape, diamondColor, clarity, cut, polish, symmetry, fluorescence, measurement, pricePerCarat, caratWeight, quantity = 1, certificate, certificateNo, supplierName, supplierId, purchasePrice, sellingPrice, discount = 0, shippingCost = 0, gstPercent = 0, paymentStatus = 'Paid', paymentMethod = 'Bank Wire', amountReceived, employeeId, salesPersonName, commissionPercent, orderStatus = 'Delivered', trackingNumber, trackingLink, dollarRate, notes, } = req.body;
        ...
        if (!finalInvoiceNo) {
            const count = await prisma_1.default.internalSale.count();
            finalInvoiceNo = `INV-${String(count + 1001).padStart(4, '0')}`;
        }
        // Check unique invoice
        const existing = await prisma_1.default.internalSale.findUnique({ where: { invoiceNo: finalInvoiceNo } });
        if (existing) {
        ...
        if (resolvedEmpId) {
            const emp = await prisma_1.default.employee.findUnique({ where: { id: resolvedEmpId } });
            if (emp)
        ...
        if (!resolvedSuppId && supplierName && supplierName.trim() !== 'None' && supplierName.trim() !== 'NONE') {
            let supp = await prisma_1.default.supplier.findUnique({ where: { name: supplierName.trim() } });
            if (!supp) {
                supp = await prisma_1.default.supplier.create({
        ...
        if (commissionPercent === undefined && resolvedEmpId) {
            const rule = await prisma_1.default.commissionRule.findFirst({
                where: {
        ...
        const sale = await prisma_1.default.internalSale.create({
            data: {
...
const updateSale = async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await prisma_1.default.internalSale.findUnique({ where: { id } });
        if (!existing) {
            return res.status(404).json({ message: 'Sale not found' });
        }
        ...
        const updated = await prisma_1.default.internalSale.update({
            where: { id },
            data: {
        ...
        if (updated.employeeId) {
            await prisma_1.default.commission.upsert({
...
const deleteSale = async (req, res) => {
    try {
        ...
        await prisma_1.default.internalSale.delete({ where: { id } });
        await prisma_1.default.activityLog.create({
            data: {
                userId: req.user?.id || 'SYSTEM',
                action: 'DELETE_SALE',
