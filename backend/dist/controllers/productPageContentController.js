...
exports.getStorefrontProductPageContent = exports.saveAdminProductPageContent = exports.getAdminProductPageContent = exports.DEFAULT_PRODUCT_PAGE_CONTENT = void 0;
const prisma_1 = __importDefault(require("../prisma"));
// Default values for initial global product page content
exports.DEFAULT_PRODUCT_PAGE_CONTENT = {
...
const getAdminProductPageContent = async (req, res) => {
    try {
        ...
        let content = await prisma_1.default.productPageContent.findFirst({
            where: { productId: targetProductId },
        });
        // If requesting product-specific content but none exists, fetch global default to prefill
        let globalContent = null;
        if (targetProductId) {
            globalContent = await prisma_1.default.productPageContent.findFirst({
                where: { productId: null },
            });
        }
        const fallbackSource = globalContent || exports.DEFAULT_PRODUCT_PAGE_CONTENT;
        if (!content) {
            // If global requested and doesn't exist in DB, create initial global DB entry
            if (!targetProductId && !globalContent) {
                content = await prisma_1.default.productPageContent.create({
                    data: {
                        productId: null,
                        ...exports.DEFAULT_PRODUCT_PAGE_CONTENT
                    }
...
exports.getAdminProductPageContent = getAdminProductPageContent;
// Admin Endpoint: Save product page content
const saveAdminProductPageContent = async (req, res) => {
    try {
        ...
        if (productId && productId !== 'global' && productId !== 'null' && String(productId).trim() !== '') {
            ...
            const product = await prisma_1.default.product.findUnique({ where: { id: targetProductId } });
            if (!product) {
        ...
        let existing = await prisma_1.default.productPageContent.findFirst({
            where: { productId: targetProductId },
        });
        let savedContent;
        if (existing) {
            savedContent = await prisma_1.default.productPageContent.update({
                where: { id: existing.id },
                data: dataToSave,
            });
        }
        else {
            savedContent = await prisma_1.default.productPageContent.create({
                data: {
                    productId: targetProductId,
                    ...dataToSave,
                },
...
exports.saveAdminProductPageContent = saveAdminProductPageContent;
// Storefront Endpoint: Get active merged product page content for PDP
const getStorefrontProductPageContent = async (req, res) => {
    try {
        const { productId } = req.params;
        // Fetch product specific content if productId provided
        let customContent = null;
        if (productId && productId !== 'undefined' && productId !== 'null') {
            customContent = await prisma_1.default.productPageContent.findFirst({
                where: { productId },
            });
        }
        // Fetch global content
        let globalContent = await prisma_1.default.productPageContent.findFirst({
            where: { productId: null },
        });
        if (!globalContent) {
            globalContent = await prisma_1.default.productPageContent.create({
                data: {
                    productId: null,
                    ...exports.DEFAULT_PRODUCT_PAGE_CONTENT
                }
...
exports.getStorefrontProductPageContent = getStorefrontProductPageContent;
