...
exports.uploadMediaFromPc = exports.downloadBulkImportErrorReport = exports.executeProductBulkUpload = exports.validateProductBulkUpload = exports.downloadProductImportTemplate = void 0;
...
const downloadProductImportTemplate = (req, res) => {
    try {
        ...
        const wsProducts = xlsx_1.default.utils.aoa_to_sheet([productsHeaders, productsSample]);
        ...
        const wsDiamonds = xlsx_1.default.utils.aoa_to_sheet([diamondsHeaders, ...diamondsSamples]);
        ...
        const wsMetals = xlsx_1.default.utils.aoa_to_sheet([metalsHeaders, ...metalsSamples]);
        // Sheet 4: Ring Sizes
        ...
        const wsSizes = xlsx_1.default.utils.aoa_to_sheet([sizesHeaders, ...sizesSamples]);
        ...
        const wsCustom = xlsx_1.default.utils.aoa_to_sheet([customHeaders, ...customSamples]);
        // Sheet 6: Accordions
        ...
        const wsAccordions = xlsx_1.default.utils.aoa_to_sheet([accordionsHeaders, ...accordionsSamples]);
        ...
        const wsInstructions = xlsx_1.default.utils.aoa_to_sheet(instructions);
        xlsx_1.default.utils.book_append_sheet(wb, wsProducts, 'Products');
        xlsx_1.default.utils.book_append_sheet(wb, wsDiamonds, 'Diamond Options');
        xlsx_1.default.utils.book_append_sheet(wb, wsMetals, 'Metal Options');
        xlsx_1.default.utils.book_append_sheet(wb, wsSizes, 'Ring Sizes');
        xlsx_1.default.utils.book_append_sheet(wb, wsCustom, 'Custom Options');
        xlsx_1.default.utils.book_append_sheet(wb, wsAccordions, 'Accordions');
        xlsx_1.default.utils.book_append_sheet(wb, wsInstructions, 'Instructions');
...
const parseMediaZip = async (zipBuffer) => {
...
const mapMetalStringToConfig = (metalStr) => {
    const s = metalStr.trim();
    if (s.includes('14K Yellow'))
        return { label: s, code: '14KY', priceAdjustment: 100 };
    if (s.includes('14K White'))
        return { label: s, code: '14KW', priceAdjustment: 100 };
    if (s.includes('14K Rose'))
        return { label: s, code: '14KR', priceAdjustment: 100 };
    if (s.includes('18K Yellow'))
        return { label: s, code: '18KY', priceAdjustment: 300 };
    if (s.includes('18K White'))
        return { label: s, code: '18KW', priceAdjustment: 300 };
    if (s.includes('18K Rose'))
        return { label: s, code: '18KR', priceAdjustment: 300 };
    if (s.includes('10K'))
        return { label: s, code: '10K', priceAdjustment: 0 };
    if (s.includes('Silver') || s.includes('925'))
        return { label: s, code: 'Ag', priceAdjustment: 0 };
    if (s.includes('Platinum'))
        return { label: s, code: 'Pt', priceAdjustment: 500 };
...
const inferCategoryFromTitle = (title) => {
    ...
    if (t.includes('wedding band') || t.includes('wedding ring'))
        return { categoryName: 'Wedding Bands', jewelryType: 'Ring' };
    if (t.includes('engagement ring') || t.includes('solitaire') || t.includes('bridal'))
        return { categoryName: 'Engagement Rings', jewelryType: 'Ring' };
    if (t.includes('earring') || t.includes('stud'))
        return { categoryName: 'Earrings', jewelryType: 'Earrings' };
    if (t.includes('necklace') || t.includes('pendant') || t.includes('chain'))
        return { categoryName: 'Necklaces', jewelryType: 'Necklace' };
    if (t.includes('bangle') || t.includes('bracelet'))
        return { categoryName: 'Bracelets', jewelryType: 'Bracelet' };
    if (t.includes('ring'))
        return { categoryName: 'Rings', jewelryType: 'Ring' };
    return { categoryName: 'Rings', jewelryType: 'Ring' };
};
// Universal Parser for Excel 7-Sheet vs Etsy CSV / Single-Sheet CSV
const parseUploadedProductWorkbook = async (buffer, mode, zipBuffer) => {
    ...
    const getSheetData = (name) => {
        ...
        if (!sheetName)
            return [];
        return xlsx_1.default.utils.sheet_to_json(workbook.Sheets[sheetName], { defval: '' });
    };
    ...
    if (rawProducts.length > 0) {
        for (let i = 0; i < rawProducts.length; i++) {
            ...
            const existingProduct = await prisma_1.default.product.findUnique({ where: { sku } });
            let isValid = true;
            const rowErrors = [];
            ...
            const productDiamonds = rawDiamonds.filter((d) => String(d.sku || d.SKU || '').trim().toLowerCase() === sku.toLowerCase());
            const productMetals = rawMetals.filter((m) => String(m.sku || m.SKU || '').trim().toLowerCase() === sku.toLowerCase());
            const productSizes = rawSizes.filter((s) => String(s.sku || s.SKU || '').trim().toLowerCase() === sku.toLowerCase());
            products.push({
                rowNum,
                name,
                sku,
                ...
                availableRingSizes: productSizes.map((s) => String(s.ring_size || '')),
                enableMetalSelection: productMetals.length > 0,
                metalsConfig: productMetals.map((m) => ({
        ...
        const firstSheetData = xlsx_1.default.utils.sheet_to_json(workbook.Sheets[firstSheetName], { defval: '' });
        ...
        for (let i = 0; i < firstSheetData.length; i++) {
            ...
            if (mediaMap.size > 0) {
                for (const [key, media] of mediaMap.entries()) {
                    if (key.includes(sku.toLowerCase())) {
                        const ext = path_1.default.extname(media.filename);
                        const savedName = `bulk_${sku}_${Date.now()}_${Math.floor(Math.random() * 1000)}${ext}`;
                        const targetPath = path_1.default.join(UPLOADS_DIR, savedName);
                        fs_1.default.writeFileSync(targetPath, media.buffer);
            ...
            for (const block of variationBlocks) {
                if (!block.name || block.vals.length === 0)
                    continue;
...
const validateProductBulkUpload = async (req, res) => {
    try {
        ...
        res.json({
            ...
            preview: products.map((p) => ({
                ...
                metal: p.metalsConfig?.map((m) => m.label || m).slice(0, 2).join(', ') || 'Standard',
                shape: 'Round/Custom',
                carat: 1.0,
                price: p.price,
                status: p.status,
...
exports.validateProductBulkUpload = validateProductBulkUpload;
// 3. EXECUTE BULK IMPORT TO DATABASE
const executeProductBulkUpload = async (req, res) => {
    try {
        const files = req.files;
        const excelFile = files?.['excel']?.[0] || files?.['file']?.[0] || files?.['excelFile']?.[0];
        const zipFile = files?.['mediaZip']?.[0] || files?.['zip']?.[0];
        ...
        for (const p of products) {
            ...
            const existing = await prisma_1.default.product.findUnique({ where: { sku: p.sku } });
            let savedProdId;
            if (existing) {
            ...
            if (p.images && p.images.length > 0) {
                await prisma_1.default.productImage.deleteMany({ where: { productId: savedProdId } });
                await prisma_1.default.productImage.createMany({
                    data: p.images.map((url, pos) => ({
...
exports.executeProductBulkUpload = executeProductBulkUpload;
const downloadBulkImportErrorReport = (req, res) => {
    res.status(404).json({ message: 'No import errors reported.' });
};
exports.downloadBulkImportErrorReport = downloadBulkImportErrorReport;
const mediaController_1 = require("./mediaController");
exports.uploadMediaFromPc = mediaController_1.uploadMediaFiles;
