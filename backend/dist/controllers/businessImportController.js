...
exports.executeSalesExcelImport = exports.validateSalesExcelImport = void 0;
const XLSX = __importStar(require("xlsx"));
const prisma_1 = __importDefault(require("../prisma"));
const validateSalesExcelImport = async (req, res) => {
    try {
        if (!req.file) {
        ...
        const existingInvoices = await prisma_1.default.internalSale.findMany({ select: { invoiceNo: true } });
...
exports.validateSalesExcelImport = validateSalesExcelImport;
const executeSalesExcelImport = async (req, res) => {
    try {
        ...
        const allSuppliers = await prisma_1.default.supplier.findMany();
        ...
        for (const r of rows) {
            try {
                ...
                if (custName) {
                    const cust = await prisma_1.default.customer.findFirst({
                        where: { name: custName },
                    });
                    if (cust) {
                        custId = cust.id;
                        ...
                        const newCust = await prisma_1.default.customer.create({
                            data: {
                                name: custName,
                                email: dummyEmail,
                ...
                if (suppName && suppName.toUpperCase() !== 'NONE' && suppName.toUpperCase() !== 'NULL') {
                    ...
                    if (supplierMap.has(key)) {
                        ...
                        const newSupp = await prisma_1.default.supplier.create({
...
exports.executeSalesExcelImport = executeSalesExcelImport;
