...
exports.getBusinessDashboardMetrics = void 0;
const prisma_1 = __importDefault(require("../prisma"));
const getBusinessDashboardMetrics = async (req, res) => {
    try {
        const period = req.query.period || 'all'; // 'today', 'month', 'year', 'all', 'custom'
        const selectedYear = req.query.year ? Number(req.query.year) : undefined;
        const selectedMonth = req.query.month || ''; // e.g. 'August' or '08'
        ...
        const [sales, salesCount, aggregate, diamondAggregate, jewelryAggregate, orderStatusGroups, paymentStatusGroups, employees, targetsAggregate,] = await Promise.all([
            ...
            prisma_1.default.internalSale.groupBy({
                by: ['orderStatus'],
                where,
                _count: { id: true },
            }),
            ...
            prisma_1.default.salesTarget.aggregate({
                where: { status: 'ACTIVE' },
        ...
        return res.json({
            ...
            orderStatusCounts: orderStatusGroups.reduce((acc, curr) => {
                acc[curr.orderStatus] = curr._count.id;
                return acc;
            }, {}),
            paymentStatusCounts: paymentStatusGroups.reduce((acc, curr) => {
