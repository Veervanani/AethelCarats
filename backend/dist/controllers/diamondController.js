...
exports.duplicateDiamond = exports.updateDiamondFilterConfig = exports.getDiamondFilterConfig = exports.deleteAllDiamonds = exports.deleteDiamond = exports.updateDiamond = exports.createDiamond = exports.updateDiamondStatus = exports.getWhatsAppInquiryMessage = exports.getDiamondById = exports.getDiamonds = void 0;
const prisma_1 = __importDefault(require("../prisma"));
const getDiamonds = async (req, res) => {
    try {
        const { type, classification, shapes, minCarat, maxCarat, colors, color, fancyColor, fancyColors, overtone, fancyOvertone, fancyOvertones, intensity, fancyIntensity, fancyIntensities, clarities, cuts, labs, minPrice, maxPrice, noBgm, search, status = 'AVAILABLE', sort = 'price-asc', page = '1', limit = '100', } = req.query;
        const pageNum = Math.max(1, parseInt(page, 10) || 1);
        const limitNum = Math.min(5000, Math.max(1, parseInt(limit, 10) || 100));
        const skip = (pageNum - 1) * limitNum;
        const where = {};
        const labQuery = (labs || req.query.certificate || req.query.lab);
        ...
        if (shapes) {
            const shapeList = shapes.split(',').map((s) => s.trim());
        ...
        if (minCarat || maxCarat) {
            where.carat = {};
            if (minCarat)
                where.carat.gte = parseFloat(minCarat);
            if (maxCarat)
                where.carat.lte = parseFloat(maxCarat);
        }
        ...
        const fancyColorParam = (fancyColors || fancyColor);
        if (fancyColorParam) {
        ...
        const overtoneParam = (fancyOvertones || overtone || req.query.fancyOvertone);
        if (overtoneParam) {
        ...
        const intensityParam = (fancyIntensities || intensity || req.query.fancyIntensity);
        if (intensityParam) {
        ...
        if (clarities) {
            const clarityList = clarities.split(',').map((c) => c.trim().toUpperCase());
            if (clarityList.length > 0) {
                where.clarity = { in: clarityList };
            }
        }
        if (cuts) {
            const cutList = cuts.split(',').map((c) => c.trim());
            if (cutList.length > 0) {
                where.cut = { in: cutList };
            }
        }
        if (labQuery) {
            const labList = labQuery.split(',').map((l) => l.trim());
            if (labList.length > 0) {
                const OR_labs = [];
                labList.forEach((l) => {
        ...
        const growthTypeParam = (req.query.growthType || req.query.growthTypes || req.query.cvdHpht);
        if (growthTypeParam) {
        ...
        const fluorParam = (req.query.fluorescence || req.query.fluorescences);
        if (fluorParam) {
        ...
        const minRatio = req.query.minRatio;
        const maxRatio = req.query.maxRatio;
        if (minRatio || maxRatio) {
            where.ratio = {};
            if (minRatio)
                where.ratio.gte = parseFloat(minRatio);
            if (maxRatio)
                where.ratio.lte = parseFloat(maxRatio);
        }
        const minPcarat = req.query.minPricePerCarat;
        const maxPcarat = req.query.maxPricePerCarat;
        if (minPcarat || maxPcarat) {
            where.pricePerCarat = {};
            if (minPcarat)
                where.pricePerCarat.gte = parseFloat(minPcarat);
            if (maxPcarat)
                where.pricePerCarat.lte = parseFloat(maxPcarat);
        }
        if (search) {
        ...
        const [diamonds, total] = await Promise.all([
            prisma_1.default.diamond.findMany({
                where,
                skip,
                take: limitNum,
                orderBy,
            }),
            prisma_1.default.diamond.count({ where }),
        ]);
        res.json({
            diamonds,
            pagination: {
        ...
        const limitNum = Math.min(5000, Math.max(1, parseInt(req.query.limit, 10) || 100));
        res.json({
            diamonds: [],
            pagination: {
                total: 0,
...
const getDiamondById = async (req, res) => {
    try {
        const { id } = req.params;
        const diamond = await prisma_1.default.diamond.findFirst({
            where: {
                OR: [{ id }, { diamondId: id }],
            },
        });
...
const getWhatsAppInquiryMessage = async (req, res) => {
    try {
        const { id } = req.params;
        const diamond = await prisma_1.default.diamond.findFirst({
...
exports.getWhatsAppInquiryMessage = getWhatsAppInquiryMessage;
const updateDiamondStatus = async (req, res) => {
    try {
        const { id } = req.params;
        const { status, price } = req.body;
        const existing = await prisma_1.default.diamond.findUnique({ where: { id } });
        if (!existing)
            return res.status(404).json({ message: 'Diamond not found' });
        const updated = await prisma_1.default.diamond.update({
            where: { id },
            data: {
...
const createDiamond = async (req, res) => {
    try {
        const data = req.body;
        const count = await prisma_1.default.diamond.count();
        const diamondId = data.diamondId || `D${10000 + count + 1}`;
        const diamond = await prisma_1.default.diamond.create({
            data: {
                diamondId,
...
const deleteDiamond = async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.default.diamond.deleteMany({
            where: {
                OR: [{ id }, { diamondId: id }],
            },
        });
...
const deleteAllDiamonds = async (req, res) => {
    try {
        const deleted = await prisma_1.default.diamond.deleteMany();
        try {
            await prisma_1.default.$executeRawUnsafe('TRUNCATE TABLE `Diamond`;');
        }
        catch (e) {
...
exports.deleteAllDiamonds = deleteAllDiamonds;
...
const getDiamondFilterConfig = async (req, res) => {
    try {
        const setting = await prisma_1.default.siteSetting.findUnique({ where: { key: 'diamond_vault_filter_config' } });
        if (setting && setting.value) {
            try {
                const parsed = JSON.parse(setting.value);
                if (parsed && Array.isArray(parsed.shapes)) {
                    const existingNames = new Set(parsed.shapes.map((s) => (s.name || s.value || '').toLowerCase()));
...
exports.getDiamondFilterConfig = getDiamondFilterConfig;
const updateDiamondFilterConfig = async (req, res) => {
    try {
...
exports.updateDiamondFilterConfig = updateDiamondFilterConfig;
const duplicateDiamond = async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await prisma_1.default.diamond.findUnique({ where: { id } });
        if (!existing)
        ...
        const duplicate = await prisma_1.default.diamond.create({
            data: {
                ...existing,
                id: undefined,
                diamondId: newId,
...
exports.duplicateDiamond = duplicateDiamond;
