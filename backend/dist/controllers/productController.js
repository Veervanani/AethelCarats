...
exports.resetDatabaseToSingleDemoProduct = exports.ensureSingleDemoProductEnforced = exports.calculateServerSidePrice = exports.deleteCustomerSpecificPrice = exports.upsertCustomerSpecificPrice = exports.getCustomerPrices = exports.updateRingSizeGuide = exports.getRingSizeGuide = exports.deleteCategory = exports.updateCategory = exports.deleteProduct = exports.duplicateProduct = exports.updateProduct = exports.createProduct = exports.createCategory = exports.getCollections = exports.getCategories = exports.getProductById = exports.getProductBySlug = exports.getProducts = void 0;
...
const getMetalSortRankNode = (label) => {
...
const mapProductResponse = (product, customerPriceRecord) => {
    ...
    if (Array.isArray(metalsConfig)) {
        metalsConfig = [...metalsConfig].sort((a, b) => {
    ...
    return {
        ...
        avgRating: product.avgRating ?? (Array.isArray(product.reviews) && product.reviews.length > 0 ? Math.round((product.reviews.reduce((s, r) => s + (r.rating || 5), 0) / product.reviews.length) * 10) / 10 : 0),
    };
};
const getProducts = async (req, res) => {
    try {
        const { category, collection, search, gender, metal, shape, style, ringSize, ringWidth, diamondType, gemstone, pearlType, birthstone, minCarat, clarity, color, cut, certification, plainMetal, onSale, newArrivals, engravable, minPrice, maxPrice, sort, page = '1', limit = '100', } = req.query;
        ...
        if (metal && metal !== 'All') {
            ...
            where.AND = where.AND || [];
            const metalOrConditions = [];
            metalTerms.forEach((mTerm) => {
                const clean = mTerm.trim().toLowerCase();
                if (clean === 'all' || clean === 'any')
                    return;
                if (clean.includes('9k')) {
                    metalOrConditions.push({ metal: { contains: '9K' } }, { goldPurity: { contains: '9K' } }, { metalsConfig: { contains: '9K' } }, { variationsJson: { contains: '9K' } });
                }
                if (clean.includes('10k')) {
                    metalOrConditions.push({ metal: { contains: '10K' } }, { goldPurity: { contains: '10K' } }, { metalsConfig: { contains: '10K' } }, { variationsJson: { contains: '10K' } });
                }
                if (clean.includes('14k')) {
                    metalOrConditions.push({ metal: { contains: '14K' } }, { goldPurity: { contains: '14K' } }, { metalsConfig: { contains: '14K' } }, { variationsJson: { contains: '14K' } });
                }
                if (clean.includes('18k')) {
                    metalOrConditions.push({ metal: { contains: '18K' } }, { goldPurity: { contains: '18K' } }, { metalsConfig: { contains: '18K' } }, { variationsJson: { contains: '18K' } });
                }
                if (clean.includes('silver')) {
                    metalOrConditions.push({ metal: { contains: 'Silver' } }, { metal: { contains: '925' } }, { goldPurity: { contains: 'Silver' } }, { metalsConfig: { contains: 'Silver' } }, { variationsJson: { contains: 'Silver' } });
                }
                if (clean.includes('yellow')) {
                    metalOrConditions.push({ metal: { contains: 'Yellow' } }, { goldColor: { contains: 'Yellow' } }, { metalsConfig: { contains: 'Yellow' } }, { variationsJson: { contains: 'Yellow' } });
                }
                if (clean.includes('white')) {
                    metalOrConditions.push({ metal: { contains: 'White' } }, { goldColor: { contains: 'White' } }, { metalsConfig: { contains: 'White' } }, { variationsJson: { contains: 'White' } });
                }
                if (clean.includes('rose')) {
                    metalOrConditions.push({ metal: { contains: 'Rose' } }, { goldColor: { contains: 'Rose' } }, { metalsConfig: { contains: 'Rose' } }, { variationsJson: { contains: 'Rose' } });
                }
                if (clean.includes('platinum')) {
                    metalOrConditions.push({ metal: { contains: 'Platinum' } }, { metalsConfig: { contains: 'Platinum' } }, { variationsJson: { contains: 'Platinum' } });
                }
                metalOrConditions.push({ metal: { contains: mTerm.trim() } }, { metalsConfig: { contains: mTerm.trim() } }, { variationsJson: { contains: mTerm.trim() } });
            });
        ...
        if (ringWidth && ringWidth !== 'All') {
            where.ringWidth = { contains: ringWidth };
        }
        if (diamondType && diamondType !== 'All') {
            ...
            if (dVal.includes('lab')) {
                ...
                where.diamondType = { contains: diamondType };
            }
        }
        if (gemstone && gemstone !== 'All') {
            where.gemstone = { contains: gemstone };
        }
        if (pearlType && pearlType !== 'All') {
            where.pearlType = { contains: pearlType };
        }
        if (birthstone && birthstone !== 'All') {
            where.birthstone = { contains: birthstone };
        }
        if (minCarat && minCarat !== 'All') {
            const caratVal = parseFloat(minCarat) || 0;
            where.AND = where.AND || [];
            where.AND.push({
                OR: [
                    { carat: { gte: caratVal } },
            ...
            if (colorVal.startsWith('fancy')) {
                ...
                where.AND.push({
                    OR: [
                        ...
                        { diamondDetailsJson: { contains: color } },
                    ],
                });
            }
        }
...
const getProductBySlug = async (req, res) => {
    try {
        ...
        const customerId = req.query.customerId || (req.user?.role === 'CUSTOMER' ? req.user.id : undefined);
        let product = null;
        try {
            product = await prisma_1.default.product.findUnique({
                where: { slug },
                include: {
                    images: { orderBy: { position: 'asc' } },
                    variants: true,
        ...
        if (customerId) {
            try {
                customerPriceRecord = await prisma_1.default.customerSpecificPrice.findUnique({
                    where: {
        ...
        let relatedProducts = [];
        try {
            relatedProducts = await prisma_1.default.product.findMany({
                where: {
    ...
    try {
        ...
        const product = await prisma_1.default.product.create({
            data: {
                ...
                style: data.style || data.ringStyle || null,
                ...
                diamondType: data.diamondType || (data.diamondDetails?.origin ? (data.diamondDetails.origin.toLowerCase() === 'natural' ? 'NATURAL' : 'LAB_GROWN') : null),
                shape: data.shape || data.diamondDetails?.shape || null,
                carat: data.carat ? parseFloat(data.carat) : (data.diamondDetails?.caratWeight ? parseFloat(data.diamondDetails.caratWeight) : null),
                color: data.color || data.diamondDetails?.color || null,
                clarity: data.clarity || data.diamondDetails?.clarity || null,
                cut: data.cut || data.diamondDetails?.cut || null,
                certificateNo: data.certificateNo || data.diamondDetails?.certificateNumber || null,
                certification: data.certification || data.diamondDetails?.certification || null,
    ...
    try {
        ...
        const updateData = {
            ...
            ...((data.style !== undefined || data.ringStyle !== undefined) && {
                style: data.style !== undefined ? data.style : data.ringStyle,
            ...
            ...(data.diamondDetails && typeof data.diamondDetails === 'object' && {
                ...(data.diamondDetails.shape && { shape: data.diamondDetails.shape }),
                ...(data.diamondDetails.caratWeight && { carat: parseFloat(data.diamondDetails.caratWeight) || null }),
                ...(data.diamondDetails.color && { color: data.diamondDetails.color }),
                ...(data.diamondDetails.clarity && { clarity: data.diamondDetails.clarity }),
                ...(data.diamondDetails.cut && { cut: data.diamondDetails.cut }),
                ...(data.diamondDetails.certification && { certification: data.diamondDetails.certification }),
                ...(data.diamondDetails.certificateNumber && { certificateNo: data.diamondDetails.certificateNumber }),
                ...(data.diamondDetails.origin && { diamondType: data.diamondDetails.origin.toLowerCase() === 'natural' ? 'NATURAL' : 'LAB_GROWN' }),
            }),
        ...
        if (data.images && Array.isArray(data.images)) {
            ...
            await prisma_1.default.productImage.createMany({ data: newImages });
...
const duplicateProduct = async (req, res) => {
    try {
        const { id } = req.params;
        const original = await prisma_1.default.product.findUnique({
            where: { id },
            include: {
                images: true,
            },
        ...
        let counter = 1;
        while (await prisma_1.default.product.findUnique({ where: { sku: newSku } })) {
            counter++;
        ...
        let slugCounter = 1;
        while (await prisma_1.default.product.findUnique({ where: { slug: newSlug } })) {
            slugCounter++;
            newSlug = `${original.slug}-copy-${slugCounter}`;
        }
        const newName = `${original.name} - Copy`;
        // Create independent duplicate product
        const duplicated = await prisma_1.default.product.create({
            data: {
                name: newName,
                sku: newSku,
                slug: newSlug,
...
const deleteProduct = async (req, res) => {
    try {
        const { id } = req.params;
        const deleted = await prisma_1.default.product.delete({ where: { id } });
        if (req.user) {
            await prisma_1.default.activityLog.create({
                data: {
                    userId: req.user.id,
...
const deleteCategory = async (req, res) => {
    try {
        const { id } = req.params;
        const deleted = await prisma_1.default.category.delete({ where: { id } });
        if (req.user) {
            await prisma_1.default.activityLog.create({
                data: {
                    userId: req.user.id,
...
// ============================================================
const getRingSizeGuide = async (req, res) => {
    try {
        let guide = await prisma_1.default.ringSizeGuide.findUnique({ where: { id: 'main' } });
        if (!guide) {
            guide = await prisma_1.default.ringSizeGuide.create({
                data: {
                    id: 'main',
...
exports.getRingSizeGuide = getRingSizeGuide;
const updateRingSizeGuide = async (req, res) => {
    try {
        const data = req.body;
        const guide = await prisma_1.default.ringSizeGuide.upsert({
            where: { id: 'main' },
            update: {
...
exports.updateRingSizeGuide = updateRingSizeGuide;
// ============================================================
...
const getCustomerPrices = async (req, res) => {
    try {
        ...
        const prices = await prisma_1.default.customerSpecificPrice.findMany({
            where,
            include: {
                customer: true,
...
const upsertCustomerSpecificPrice = async (req, res) => {
    try {
        ...
        const priceRecord = await prisma_1.default.customerSpecificPrice.upsert({
            where: {
...
exports.upsertCustomerSpecificPrice = upsertCustomerSpecificPrice;
const deleteCustomerSpecificPrice = async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.default.customerSpecificPrice.delete({ where: { id } });
...
exports.deleteCustomerSpecificPrice = deleteCustomerSpecificPrice;
// ============================================================
// SERVER-SIDE PRICE CALCULATION (SECURITY ENFORCED)
// ============================================================
const calculateServerSidePrice = async (req, res) => {
    try {
        ...
        if (metalLabel) {
            ...
            if (customMetalRules[metalLabel] !== undefined) {
                ...
                const foundMetal = metalsConfig.find((m) => m.label === metalLabel);
                if (foundMetal) {
        ...
        if (diamondId) {
            ...
            if (customDiamondRules[diamondId] !== undefined) {
                ...
                const dbDiamond = await prisma_1.default.diamond.findUnique({ where: { id: diamondId } });
                if (dbDiamond) {
                    diamondPrice = dbDiamond.price;
                }
                else {
                    const diamondsConfig = safeJsonParse(product.diamondsConfig, []);
                    const foundDiamond = diamondsConfig.find((d) => d.id === diamondId);
                    if (foundDiamond) {
...
const ensureSingleDemoProductEnforced = async () => {
    try {
        const productCount = await prisma_1.default.product.count();
        if (productCount === 0) {
            console.log('🌱 Seeding initial demo product for empty database...');
            let ringsCategory = await prisma_1.default.category.findFirst({
                where: { OR: [{ slug: 'rings' }, { name: 'Rings' }] },
            });
            if (!ringsCategory) {
                ringsCategory = await prisma_1.default.category.create({
                    data: {
                        name: 'Rings',
                        slug: 'rings',
...
exports.ensureSingleDemoProductEnforced = ensureSingleDemoProductEnforced;
const resetDatabaseToSingleDemoProduct = async (req, res) => {
    try {
        ...
        await prisma_1.default.customerSpecificPrice.deleteMany({});
        await prisma_1.default.productImage.deleteMany({});
        await prisma_1.default.productVideo.deleteMany({});
        await prisma_1.default.productVariant.deleteMany({});
        const deleteResult = await prisma_1.default.product.deleteMany({});
        await (0, exports.ensureSingleDemoProductEnforced)();
        const count = await prisma_1.default.product.count();
        const demo = await prisma_1.default.product.findUnique({ where: { sku: 'FJ-DEMO-RING-001' } });
        res.json({
            success: true,
...
exports.resetDatabaseToSingleDemoProduct = resetDatabaseToSingleDemoProduct;
