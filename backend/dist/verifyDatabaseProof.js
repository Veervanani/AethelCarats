...
async function verifyDatabaseProof() {
    ...
    const mayfairRowsToClean = await prisma_1.default.siteSetting.findMany({
        where: {
            OR: [
    ...
    for (const row of mayfairRowsToClean) {
        ...
        if (row.key === 'footer_settings' || row.key === 'footer_config' || row.key === 'site_settings' || row.key === 'consult_expert_config') {
            try {
                ...
                if (obj.address)
                    obj.address = 'Surat, India';
                if (obj.storeAddress)
                    obj.storeAddress = 'Surat, India';
                if (obj.copyrightText)
                    obj.copyrightText = '© 2026 Aura Diamond Atelier. All Rights Reserved.';
                if (obj.brandName)
                    obj.brandName = 'AURA DIAMOND ATELIER';
                if (obj.consultDescription)
                    obj.consultDescription = obj.consultDescription.replace(/Mayfair/g, 'Surat');
    ...
    const remainingMayfairRows = await prisma_1.default.siteSetting.findMany({
        where: {
            OR: [
                { value: { contains: 'Mayfair' } },
                { value: { contains: 'London' } }
    ...
    const remainingMayfairProductContents = await prisma_1.default.productPageContent.findMany({
        where: {
            OR: [
