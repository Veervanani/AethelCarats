...
exports.cleanupDatabaseSiteSettings = cleanupDatabaseSiteSettings;
const prisma_1 = __importDefault(require("./prisma"));
async function cleanupDatabaseSiteSettings() {
    try {
        ...
        const footerRow = await prisma_1.default.siteSetting.findUnique({ where: { key: 'footer_settings' } });
        let footerObj = {
        ...
        const footerConfigRow = await prisma_1.default.siteSetting.findUnique({ where: { key: 'footer_config' } });
        if (footerConfigRow?.value) {
            try {
                ...
                if (Array.isArray(parsedConfig.socialLinks)) {
                    parsedConfig.socialLinks = parsedConfig.socialLinks.map((link) => {
        ...
        const siteSettingsRow = await prisma_1.default.siteSetting.findUnique({ where: { key: 'site_settings' } });
        ...
        const consultConfigRow = await prisma_1.default.siteSetting.findUnique({ where: { key: 'consult_expert_config' } });
        ...
        const allProductContents = await prisma_1.default.productPageContent.findMany();
        for (const pc of allProductContents) {
            ...
            if (needsUpdate) {
                await prisma_1.default.productPageContent.update({
                    where: { id: pc.id },
                    data: {
                        heroTitle,
